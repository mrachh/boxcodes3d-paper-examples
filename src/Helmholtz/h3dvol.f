...
c
      ...
      subroutine h3dtaevalgridmatp_fast(zk,rscale,nterms,bs,
     1     n,wlege,nlege,mat,ldmat)
c
c     Form the matrix mapping a local expansion on a box to values
c     of the corresponding potential at a tensor Legendre grid
...
c     local
      ...
      complex *16 ztmp1,ztmp2,ztmp3,ztmpsum,z,fjder
      integer ifder,itarg,m

      complex *16 zero, one
      data zero / (0.0d0,0.0d0) /
      data one / (1.0d0,0.0d0) /

      allocate(fjs(0:nterms),ynm(0:nterms,0:nterms))
...
c     use vectorized routine to fill in (do by hand later if too slow)
      ...
      do itarg=1,n3
        call cart2polar(x(1,itarg),r,theta,phi)
        ctheta = dcos(theta)
        stheta= dsin(theta)
        cphi = dcos(phi)
        sphi = dsin(phi)
...
c
        ...
        do m=-nterms,nterms
          do i=0,nterms
            if(abs(m).le.i) then
              ii = (m+nterms)*(nterms+1) + i+1
              pot(ii,itarg) = fjs(i)*ynm(i,abs(m))*ephi(m)
            endif
          enddo
        enddo
      enddo
